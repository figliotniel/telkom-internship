<x-app-layout>
                ...
                <div onclick="window.location='{{ route('admin.divisions.show', $division->id) }}'" class="cursor-pointer division-card bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 {{ $scheme['border'] }} hover:shadow-xl {{ $scheme['shadow'] }} transition-all duration-300 relative overflow-hidden group flex flex-col h-full hover:-translate-y-1">
                    ...
                    <div class="flex justify-between items-start mb-5">
                        ...
                        <div class="flex gap-2 relative z-20">
                            <!-- Edit Action (Index card edit) -->
                            <button onclick="event.stopPropagation(); openModal('edit', '{{ $division->id }}', '{{ $division->code }}', '{{ $division->name }}', '{{ $division->mentor_id }}')" class="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 transition-colors" title="Edit Divisi">
                            ...
                            <form action="{{ route('admin.divisions.destroy', $division->id) }}" method="POST" class="m-0">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="event.stopPropagation(); confirmDelete(this, '{{ $division->name }}')" class="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 transition-colors" title="Hapus Divisi">
    ...
    <script>
        ...
        function confirmDelete(button, name) {
            Swal.fire({
                title: 'Hapus Divisi?',
                html: `Anda yakin ingin menghapus divisi <b>${name}</b>?`,
                icon: 'warning',
