                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-4 group">
                                                <div class="relative flex-shrink-0">
                                                    <div class="h-12 w-12 rounded-2xl bg-gradient-to-tr from-emerald-400/20 to-teal-500/20 dark:from-emerald-500/10 dark:to-teal-400/10 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-black text-xl shadow-sm border border-emerald-100 dark:border-emerald-500/20 transition-transform group-hover:scale-110">
                                                        {{ substr($internship->student->name, 0, 1) }}
                                                    </div>
                                                </div>
                                                <div class="flex flex-col min-w-0">
                                                    <div class="text-sm font-bold text-slate-800 dark:text-slate-100 transition-colors group-hover:text-emerald-600 dark:group-hover:text-emerald-400 truncate max-w-[180px]" title="{{ $internship->student->name }}">
                                                        {{ $internship->student->name }}
                                                    </div>
                                                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 tracking-wider truncate max-w-[180px]" title="{{ $internship->student->email }}">
                                        ...
                                        @if($status !== 'pending')
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex flex-col items-center justify-center min-w-[140px]">
                                                    <div class="text-[11px] font-bold text-slate-800 dark:text-slate-200 truncate max-w-[130px] transition-colors" title="{{ $internship->division?->name ?? '-' }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                                                <div class="flex flex-col items-center justify-center min-w-[140px]">
                                                    <div class="text-[11px] font-bold text-slate-800 dark:text-slate-200 truncate max-w-[130px] transition-colors" title="{{ $internship->mentor?->name ?? '-' }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                @if($status === 'active')
                                                    ...
                                                    <div class="flex items-center justify-center">
                                                        <div class="flex items-center gap-3 text-left bg-slate-50/50 dark:bg-slate-800/50 px-3 py-2 rounded-xl border border-slate-100 dark:border-slate-700/50">
                                                            <div class="flex-shrink-0">
                                                                <div class="p-1.5 rounded-lg {{ $remainingDays > 10 ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400' : ($remainingDays > 0 ? 'bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400' : 'bg-slate-200 dark:bg-slate-700 text-slate-500') }} transition-colors">
                                                            ...
                                                            <div class="flex flex-col">
                                                                ...
                                                                @elseif($remainingDays == 0)
                                                                    <div class="text-[11px] font-black text-orange-600 dark:text-orange-400 leading-none">Last Day</div>
                                                                    <div class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Today</div>
                                                                @else
                                                                    <div class="text-[11px] font-black text-slate-500 dark:text-slate-600 leading-none">Completed</div>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
                                                @elseif($status === 'extension')
                                                    ...
                                                    @endphp
                                                    <div class="flex items-center justify-center">
                                                        <div class="flex flex-col items-center bg-emerald-50/50 dark:bg-emerald-500/10 px-3 py-2 rounded-xl border border-emerald-100 dark:border-emerald-500/20">
                                                            <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">Extended To</div>
                                                            <div class="text-[12px] font-black text-emerald-600 dark:text-emerald-400 mt-1">
                                                                {{ \Carbon\Carbon::parse($extension->new_end_date)->format('d M Y') }}
                                                            </div>
                                                            <div class="mt-1 flex items-center gap-1.5">
                                                                <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500">From {{ \Carbon\Carbon::parse($internship->end_date)->format('d M') }}</span>
                                                                <span class="inline-flex items-center px-1.5 py-0.5 rounded-lg text-[8px] font-black bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20">
                                                                    +{{ \Carbon\Carbon::parse($extension->new_start_date)->diffInDays(\Carbon\Carbon::parse($extension->new_end_date)->addDay()) }}D
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @else
                                                    <div class="flex items-center justify-center">
                                                        <div class="flex flex-col items-center">
                                                            <div class="text-[11px] font-black text-slate-800 dark:text-slate-200 tracking-tight transition-colors">
                                                        ...
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            @if($signedPact)
                                                                <a href="{{ Storage::url($signedPact->file_path) }}" target="_blank" 
                                                                   class="inline-flex items-center justify-center p-2 rounded-xl text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700/50 shadow-sm" title="Lihat Pakta">
                                                                ...
                                                                <button @click="$dispatch('open-activation-modal', { id: {{ $internship->id }}, name: '{{ $internship->student->name }}' })" 
                                                                    class="inline-flex items-center px-4 py-2 text-[10px] uppercase tracking-widest font-black rounded-xl text-white bg-emerald-500 hover:bg-emerald-600 shadow-md shadow-emerald-500/20 transition-all active:scale-95">
                                                                    Activate
                                                                </button>
                                                            @else
